...
export default function TestPage() {
    ...
    const getLevel = (score: number) => {
        if (score <= 3) return { level: 'Beginner (A1)', course: 'kids' } // Or adults beginner
        if (score <= 6) return { level: 'Elementary (A2)', course: 'adults' }
        if (score <= 8) return { level: 'Intermediate (B1)', course: 'adults' }
        return { level: 'Upper-Intermediate (B2)+', course: 'ielts' }
    }
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[var(--background)]">
            <Header translations={{}} locale="ru" onLocaleChange={() => { }} />
                            ...
                            <div className="grid gap-4">
                                {questions[currentQuestion].options.map((option, index) => (
                                    <button
                                        ...
                                        className="p-4 rounded-xl border-2 border-[var(--border)] bg-[var(--surface)] text-[var(--text)] text-left hover:border-[var(--primary)] hover:bg-[var(--primary)]/5 transition-all font-medium text-lg"
                                    >
                                        {option}
                                    </button>
                                ))}
                    ...
                    {step === 'result' && (
                        <Card className="p-8 text-center animate-fadeIn">
                            ...
                            <div className="bg-[var(--surface-hover)] p-6 rounded-2xl mb-8">
                                ...
                                <p className="text-xl font-bold text-[var(--text)]">Current Course</p>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                <Button href="/" variant="outline" leftIcon={<RefreshCcw size={18} />}>
                                    На главную
                                </Button>
                                <a href="https://wa.me/77020296315?text=Здравствуйте! Я прошел тест, мой уровень примерно ${result.level}. Хочу записаться на пробный урок." target="_blank" rel="noopener noreferrer">
